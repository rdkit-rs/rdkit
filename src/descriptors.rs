...
impl Properties {
    ...
    pub fn compute_properties(&self, ro_mol: &ROMol) -> HashMap<String,f64> {
        ...
        let computed = rdkit_sys::descriptors_ffi::compute_properties(self.ptr.clone(), ro_mol.ptr.clone());

        assert!(names.len() != 0);
        assert!(computed.len() == names.len());

        names.into_iter().zip(computed.into_iter()).map(|(k,v)| (k.to_string(),*v)).collect()
    }
}